...
function parseCatalog(){
  ...
  return text.split('\n').map(x=>x.trim()).filter(Boolean).map((x,i)=>{
    ...
    const k=p.r+'|'+p.n;
    if(!p.r||!p.n||!p.t||!Number.isFinite(p.q)||seen.has(k))return false;
...
const players=catalog.map(p=>{const old=oldMap.get(p.r+'|'+p.n);return old?{...p,status:old.status||'free',paid:Number(old.paid)||0}:{...p}});
let st={
  budget:Number(previous?.budget)||500,
  slots:{P:Number(previous?.slots?.P)??3,D:Number(previous?.slots?.D)??8,C:Number(previous?.slots?.C)??8,A:Number(previous?.slots?.A)??6},
  players,
  closed:Array.isArray(previous?.closed)?previous.closed.filter(r=>ROLE_NAMES[r]):[],
  view:previous?.view||'P',
  minQ:Number(previous?.minQ)||1
};
if(!['P','D','C','A','MINE','OTHERS','SET'].includes(st.view))st.view='P';
function save(){localStorage.setItem(KEY,JSON.stringify(st))}
save();
function mine(){return st.players.filter(p=>p.status==='mine')}
function totalSlots(){return Object.values(st.slots).reduce((a,b)=>a+(Number(b)||0),0)}
function credits(){return st.budget-mine().reduce((a,p)=>a+(Number(p.paid)||0),0)}
...
function choose(p){
  ...
  $('#confirm').onclick=()=>{
    ...
    if(roleMineCount(p.r)>=Number(st.slots[p.r]))return alert('Slot '+p.r+' già completi');
...
function render(){
  ...
  $('#squad').textContent=m.length+'/'+totalSlots();
  $('#maxBid').textContent=maxBid();
  document.querySelectorAll('[data-view]').forEach(b=>{
    b.classList.toggle('active',b.dataset.view===st.view);
    if(['P','D','C','A'].includes(b.dataset.view))b.classList.toggle('hidden',st.closed.includes(b.dataset.view));
  });
  const isRole=['P','D','C','A'].includes(st.view);
  $('#roleActions').classList.toggle('hidden',!isRole);
  $('#search').classList.toggle('hidden',st.view==='SET');
  $('#filterWrap').classList.toggle('hidden',!isRole);
  $('#minQ').value=String(st.minQ||1);
  $('#list').innerHTML='';$('#action').innerHTML='';
  if(st.view==='SET'){
    $('#info').innerHTML=settingsHtml();bindSettings();return;
  }
  ...
  arr.forEach(p=>{
    ...
    b.innerHTML=`<span class="playerText"><span class="pn">${escapeHtml(p.n)}</span><span class="meta">${escapeHtml(p.t)} · ${p.r}${p.status==='mine'?' · pagato '+p.paid:''}</span><span class="grade ${gradeClass(p.g)}">${p.g}</span></span><span class="quote">${p.q}</span>`;
    b.onclick=()=>choose(p);$('#list').appendChild(b);
  });
}
document.querySelectorAll('[data-view]').forEach(b=>b.onclick=()=>{st.view=b.dataset.view;$('#search').value='';save();render()});
$('#search').oninput=render;
$('#minQ').onchange=()=>{st.minQ=Number($('#minQ').value)||1;save();render()};
